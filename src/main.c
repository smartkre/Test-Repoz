...
// --- Инициализация тактирования с внешним HSE (8 МГц) ---
...
// --- Настройка TIM2 для ШИМ на PA2 (TIM2_CH3, ~4 МГц, 50%) ---
void TIM2_Init(void) {
    // Настройка PA2 как AF push-pull (для TIM2_CH3)
    GPIOA->CRL &= ~(0xF << (2 * 4));  // Очищаем PA2 (сдвиг 2)
    GPIOA->CRL |= (0xB << (2 * 4));   // AF push-pull 10MHz

    // Настройка TIM2 для частоты ~4 МГц:
    // Тактовая частота таймера = 8 МГц (HSE)
    // Делитель предварительного счета (PSC) = 0 (без деления)
    // Период (ARR) = 1 -> частота = 8 МГц / (1 + 1) = 4 МГц
    TIM2->PSC = 0;     // Без деления (частота таймера = 8 МГц)
    TIM2->ARR = 1;     // Период = 2 такта -> частота = 8 МГц / 2 = 4 МГц
    TIM2->CCR3 = 1;    // Скважность 50% (1 / 2 = 0.5)
...
int main(void) {
    System_Init();
    TIM2_Init();  // Генерация ШИМ ~4 МГц на PA2

    while (1) {
        // Ничего не делать, ШИМ работает автоматически
        // Частота: 4 МГц (±), скважность: 50%
    }
}
